...
let's see how the prediction improves after altering  the equation $Y~u,i~ = μ + b~i$
```{r}
...
Notice that there is substantial variability across users ratings as well. This implies that a further improvement to our model may be $Y~u,i~ = μ + b~i~ + ε~u,i~$ we could fit this model by using use the lm() function but as mentioned earlier it would be very slow $lm(rating ~ as.factor(movieId) + as.factor(userId))$
so here is the code 
```{r}
user_avgs <- train_set %>% 
  left_join(movie_avgs, by='movieId') %>%
